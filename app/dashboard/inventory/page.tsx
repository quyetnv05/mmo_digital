...
export default function InventoryPage() {
    ...
    // Load products (mock for now)
    useEffect(() => {
        // TODO: Fetch real products from API
        setProducts([
            { id: 1, name: 'Clone Facebook 2FA', price: 15000, _count: { items: 150 } },
            { id: 2, name: 'Gmail PVA', price: 5000, _count: { items: 500 } },
            { id: 3, name: 'Tiktok Aged Account', price: 20000, _count: { items: 75 } },
        ]);
    }, []);
                        ...
                        >
                            ...
                            {products.map((product) => (
                                <option key={product.id} value={product.id}>
                                    {product.name} ({product._count?.items || 0} trong kho)
                                </option>
                            ))}
                        </select>
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-green-400 font-semibold">
                                    {products.reduce((sum, p) => sum + (p._count?.items || 0), 0)}
                                </span>
                            </div>
